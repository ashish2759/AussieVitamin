...
class EditCart extends MY_Controller
{
    ...
    function set(){
        	...
        	if(!$this->session->userdata('user_type')) {
				##$this->session->set_userdata(['add_to_cart_id' => $this->input->get('ip'), 'add_to_cart_qty' => $this->input->get('qty')]);
                ##redirect('login');

            }
	
         $check_product_id = $this->Publicmodel->getSingleTranslation(['id' => $this->input->get('ip'), 'type' => 'product']);
//pr($check_product_id,0);
        if($check_product_id){
...
//pr(get_cookie('product_id'.$i) .'!=='.$this->input->get('ip'),0);
...
//pr(get_cookie('i'.$i),0);
//pr(get_cookie('product_id'.$i),0);
//pr(get_cookie('qty'.$i),0);
//pr(get_cookie('total'),0);

...
class Checkout extends MY_Controller
{
	...
	prf(get_cookie('total'),'order_item','order_item');					  
	...
	prf(array($res1,$total),'order_item','order_item');					  
                          $totals += $total;
                      		}
			}
...
Array
(
    [__ci_last_regenerate] => 1512673381
    [referrer] => Direct
    [user_id] => 15
    [user_type] => customer
    [username] => test001
...
*/		
			$Params	= array();
			$Params['to'] = $to = $this->session->userdata('email');
			$Params['subject'] = $subject = 'Aussie Vitamin Store: Order ID #' . $orderId;
            $Params['txt'] = $txt = 'Hello ' . $this->session->userdata('name') . ',<br><br>Thank you for placing the order at Aussie Vitamin Store. Your order id #' . $orderId. ' . <br><br> <br><br>Thank you.';
			$this->send_mail($Params);
				
			$Params	= array();
			$Params['to'] = $to = 'admin@aussievitaminstore.com';
			$Params['subject'] = $subject = 'Aussie Vitamin Store: Order ID #' . $orderId.' from '.$this->session->userdata('name');
            $Params['txt'] = $txt = 'Hello,  <br><br>' . $this->session->userdata('name') . ' has placed the order at Aussie Vitamin Store. The order id #' . $orderId. ' . <br><br> ';
			$this->send_mail($Params);

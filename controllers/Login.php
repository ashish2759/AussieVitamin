...
$this->guest_to_cart_afterlogin(array('user_id'=>$user_data[0]['user_id']));
...
$this->guest_to_cart_afterlogin(array('user_id'=>$user_data[0]['user_id']));
    ...
    public function guest_to_cart_afterlogin($Params	= array())
    {
		...
		prf($Params,'','session_id_val');
		$session_id = $this->session->userdata('__ci_last_regenerate');
		prf($session_id,'','session_id_val');
                	$update_refferer = $this->Publicmodel->update_cart(

...
class Loop
{
    ...
    static function getCartItems($cartItems)
    {
        ...
        <?php } else {
            ?>
            <li class="text-center" style="color:black"><?= lang('no_products') ?></li>
            <?php
        }
    }
...
//    [id] => 256
//    [quantity] => 72
//    [image] => DSZ-00222-1.jpg
//    [url] => Doggie_Sudz_Shampoo_for_Pampering_Pooch_Mango__Neem_16_fl_oz_472_ml_256
//    [price] => 38.35
//    [title] => Doggie Sudz, Shampoo for Pampering Pooch, Mango & Neem, 16 fl oz (472 ml)
//    [old_price] => 44.98
//)				
                    ...
                    <div class="product-list <?= $carousel == true ? 'item' : '' ?> <?= $classes ?> <?= $active ?>">
                        <div class="inner">
                            ...
                            <div class="add-to-cart">
                                <a href="<?= site_url('editCart/set?ip=').$article['id'].'&qty='.$article['quantity']; ?>" class="add-to-cart btn-add" data-goto="<?= LANG_URL . '/shopping-cart' ?>" data-id="<?= $article['id'] ?>">
                                    <img class="loader" src="<?= base_url('assets/imgs/ajax-loader.gif') ?>" alt="Loding">
                                    <span class="text-to-bg"><?= lang('add_to_cart') ?></span>
                                </a>
                            </div>
                            <div class="add-to-cart">
                                <a href="<?= site_url('editCart/set?ip=').$article['id'].'&qty='.$article['quantity']; ?>" class="add-to-cart btn-add more-blue" data-goto="<?= LANG_URL . '/checkout' ?>" data-id="<?= $article['id'] ?>">
                                    <img class="loader" src="<?= base_url('assets/imgs/ajax-loader.gif') ?>" alt="Loding">

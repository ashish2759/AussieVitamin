...
class AdminModel extends CI_Model
{
    ...
    public function getAdminUsers($data, $user = null)
    {
    ...
    public function setProductTranslation($post, $id, $is_update)
    {
        ...
        $getmanufacturer = $this->getsinglemanufacturer(['manufacturer_id' => $post['manufacturer']]);
        foreach ($post['abbr'] as $abbr) {
            ...
            $arr = array(
                ...
                'manufacturer' => $getmanufacturer[0]['manufacturer_name'],
                'description_r' => $post['description_r'][$i],
                'product_code' => $post['product_code'],
                'expiration_date' => $post['expiration_date'],
                'upc_code' => $post['upc_code'],
                'dimension' => $post['dimension'], 
                'source_url' => $post['source_url'],
                'weight' => $post['weight'],
                'package_qty' => $post['package_qty'],
                'product_added_date' => date('Y-m-d h:i:s'),
                'product_added_by' => $this->session->userdata('admin_id')
            );
    ...
    public function getProducts($limit, $page, $search_title = null, $orderby = null, $category = null)
    {
        ...
        $this->db->where('translations.type', 'product');
        $this->db->where('translations.abbr', MY_DEFAULT_LANGUAGE_ABBR);
        $query = $this->db->select('products.*, translations.title, translations.description,translations.description_r, translations.product_code, translations.expiration_date,translations.upc_code, translations.source_url, translations.cost_price, translations.price, translations.old_price, translations.abbr, products.url, translations.for_id, translations.type, translations.basic_description, translations.expiration_date, translations.upc_code')->get('products', $limit, $page);
        return $query;
    }
    ...
    public function getTranslations($id, $type)
    {
        ...
        foreach ($query->result() as $row) {
            ...
            $arr[$row->abbr]['cost_price'] = $row->cost_price;
    ...
    public function changeOrderStatus($id, $to_status, $status_details)
    {
        $this->db->where('id', $id);
        ...
        if ($res['processed'] != $to_status) {
            $this->db->where('id', $id);
            $result = $this->db->update('orders', array('processed' => $to_status, 'status_info' => $status_details, 'viewed' => '1' ));
            if ($result == true) {
    ...
    public function EditTrackingNumber($id,   $TrackingNumber)
    {
            $this->db->where('id', $id);
            $result = $this->db->update('orders', array('TrackingNumber' => $TrackingNumber, 'viewed' => '1' ));
		if ($result == true) {
        } else {
            $result = false;
        }
...
//        return $result;
    ...
    public function deleteManufacturer($data){
        $this->db->where($data);
        $this->db->delete('manufacturer');
        return $this->db->affected_rows();
    }
    
    public function getsinglemanufacturer($id){
        $result = $this->db->get_where('manufacturer', $id);
        return $result->result_array();
    }
    
    public function get_order_client($data) {
        $this->db->select('orders_clients.email as email, orders_clients.first_name as first_name, orders.order_id as order_id, orders.processed as processed');     
        $this->db->join('orders_clients', 'orders.id = orders_clients.for_id');
        $query = $this->db->get_where('orders', $data);
        return $query->result_array();
    }
    
    public function getStaffUsers($user = null)
    {
        if ($user != null && is_numeric($user)) {
            $this->db->where('id', $user);
        } else if ($user != null && is_string($user)) {
    ...
    {
        $this->db->where('id', $id);
        $result = $this->db->delete('staff_users');
        return $result;
    }
    
    public function setStaffUser($id)
    {
        if ($post['edit'] > 0) {
            if (trim($post['password']) == '') {
                unset($post['password']);
            ...
            unset($post['edit']);
            $post['password'] = md5($post['password']);
            $result = $this->db->insert('staff_users', $post);
        }
        return $result;
    }
    ...
    public function staff_get($id){
        $result = $this->db->get_where('users', $id);
        return $result->result_array();
    }
    
    public function customer_get($data){
